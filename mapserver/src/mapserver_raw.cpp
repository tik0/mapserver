...
static double rate;
static double resolution_meterPerTile;
static float maxDistanceInsertion_m;
static float maxX_m;
static float minX_m;
static float maxY_m;
static float minY_m;
static int debug = 0, debugDrawRpc = 0;
static int sendTopLayerOfDistanceAsOgm;
static int mapSizeX_tiles, mapSizeY_tiles;
static float mapSizeX_m, mapSizeY_m;
static cv::Mat storageMapBuffer;
static int maxLayer;
static std::shared_ptr<std::vector<cv::Mat>> currentMapHeight_mm, lastMapHeight_mm;
static std::shared_ptr<std::vector<cv::Mat>> currentPulsewidth_ps, lastPulsewidth_ps;
static std::shared_ptr<std::vector<cv::Mat>> currentMapIterator, lastMapIterator;
static cv::Mat mapIterator_1, mapIterator_2;  // Pointer to the current position in the mapHeight map
static int dontStoreMaps = 0, shiftMap = 1;
static std::string mapStorageLocation = "/tmp";

// Debug stuff for service RPC
static cv::Mat mapStackStatisticRequestDebug, mapStackStatisticDebug;
...
///
template <typename T>
void mapRefreshAndStorage(std::shared_ptr<std::vector<cv::Mat>> mapStack,
  ...
  if (storeMapStack) {
    ...
    for (int mapIdx = 0; mapIdx < maxLayer; ++mapIdx) {
      ...
      const std::string format = formatString.empty() ?
      ...
      oss << mapStorageLocation
          ...
          << "LayerIdx_" << mapIdx << "_"
          << "GridRes_" << resolution_meterPerTile * geometry::millimeterPerMeter <<  "mm_"
          << "X_" << transformRoiInWorld.getOrigin().x() << "m_"
          << "Y_" << transformRoiInWorld.getOrigin().y() << "m_"
          << "Z_" << transformRoiInWorld.getOrigin().z() << "m_"
          << "rows_" << mapStack->at(mapIdx).rows << "_"
          << "cols_" << mapStack->at(mapIdx).cols << "_"
          << ".bin";

      ROS_DEBUG("Store map to: %s\n"
                "With format: %s\n", oss.str().c_str(), utils::conversion::format2str(mapStack->at(mapIdx).type()).c_str());

      // Store the layer
      std::ofstream f;
      f.open(oss.str(), std::ofstream::out | std::ofstream::binary);
      if (f.is_open()) {
         f.write((char*)mapStack->at(mapIdx).data,
                  mapStack->at(mapIdx).rows * mapStack->at(mapIdx).cols * utils::conversion::type2size(mapStack->at(mapIdx).type()));
         f.close();
      } else {
  ...
  if (shiftMapStack) {
    ...
    ROS_DEBUG("World (x,y,z): %f, %f, %f", transformRoiInWorld.getOrigin().x(), transformRoiInWorld.getOrigin().y(), transformRoiInWorld.getOrigin().z());
    ROS_DEBUG("Last (x,y,z): %f, %f, %f", transformRoiInWorldLast.getOrigin().x(), transformRoiInWorldLast.getOrigin().y(), transformRoiInWorldLast.getOrigin().z());

    const double xshift_tiles = -(transformRoiInWorldLast.getOrigin().x() - transformRoiInWorld.getOrigin().x()) / resolution_meterPerTile;
    const double yshift_tiles = -(transformRoiInWorldLast.getOrigin().y() - transformRoiInWorld.getOrigin().y()) / resolution_meterPerTile;

    ROS_DEBUG("Shift of the map measured in tiles: x= %d, y= %d", int(xshift_tiles), int(yshift_tiles));

    for (std::size_t idx = 0; idx < mapStack->size(); ++idx) {
      translateMap(mapStack->at(idx), mapStackShiftedResult->at(idx), xshift_tiles, yshift_tiles, fillValue);
    }
...
void dataHandler(const sensor_msgs::LaserScan scanMsg)
{
  ...
  mtxSwap.lock();
    const std::string frameTarget(currentTileTfName + tileOriginTfSufixForRoiOrigin);
  ...
  projector.projectLaser(scanMsg, cloud, -1.0, laser_geometry::channel_option::Intensity);
  try {
    listenerTf->waitForTransform(frameTarget, cloud.header.frame_id, cloud.header.stamp, ros::Duration(0.3));
    listenerTf->transformPointCloud(frameTarget, cloud, cloudRoi);
  } catch(const std::exception &exc) {
    const std::string excStr(exc.what());
    ROS_WARN("%s, frameTarget:%s, cloud.header.frame_id:%s\n", excStr.c_str(), frameTarget.c_str(), cloud.header.frame_id.c_str());
    return;
  }
...
//    }
    ...
    if (cloudRoi.points.at(idx).x > mapSizeX_m || cloudRoi.points.at(idx).x < 0.0f ||
        cloudRoi.points.at(idx).y > mapSizeY_m || cloudRoi.points.at(idx).y < 0.0f ) {
      ROS_DEBUG("Skip value out of map");
      continue;
    }

    // Assigne the data
    const int x_px = static_cast<int>(floor(cloudRoi.points.at(idx).x/resolution_meterPerTile));
    const int y_px = static_cast<int>(floor(cloudRoi.points.at(idx).y/resolution_meterPerTile));
    const double z = cloudRoi.points.at(idx).z; // The height lies in the z-axis of the ROI frame

    mapHeight_mm->at(mapIterator->at(0).at<uint8_t>(y_px, x_px)).at<int16_t>(y_px, x_px) = static_cast<int16_t>(z * geometry::millimeterPerMeter);
    pulsewidth_ps->at(mapIterator->at(0).at<uint8_t>(y_px, x_px)).at<int16_t>(y_px, x_px) = static_cast<int16_t>(int16_t(cloudRoi.channels.at(0).values.at(idx)));
...
///
void getStatistic(const std::vector<cv::Mat> &src, cv::Mat &dst, const statistics statisticReq, const uint8_t quantil = 0/*1 .. 99 %*/) {
  ...
  if (statisticReq == statistics::variance) {
    ...
    if (quantil < 1 || quantil > 99) {
      throw std::runtime_error(std::string("getStatistic: Unsupported quantil"));
    }
    getStatistic(src, mean, statistics::mean);
  } else {
    throw std::runtime_error(std::string("getStatistic: Unsupported statistic type"));
  }

  cv::Mat dstTmp = cv::Mat(src.at(0).rows,
                   src.at(0).cols,
                   CV_32SC1, cv::Scalar_<int32_t>(0));

  dst = cv::Mat(src.at(0).rows,
                src.at(0).cols,
                src.at(0).type());

  cv::Mat validCounter = cv::Mat(src.at(0).rows,
                                 src.at(0).cols,
                                 CV_32SC1, cv::Scalar_<int32_t>(0));
  ...
  if (statisticReq == statistics::variance || statisticReq == statistics::quantil) {
  ...
  if (statisticReq == statistics::quantil) {
    ...
    for (int idxTile = 0; idxTile < dstTmp.rows * dstTmp.cols; ++idxTile) {
      ...
      dstTmp.at<int32_t>(idxTile) = static_cast<int32_t>(erf_inv_z * sqrt(2.0 * var) + mu);
    }
  }

  // Copy back the result
  for (int idxTile = 0; idxTile < src.at(0).rows * src.at(0).cols; ++idxTile) {
    dst.at<int16_t>(idxTile) = static_cast<int16_t>(dstTmp.at<int32_t>(idxTile));
  }
}
...
///
cv::Mat getStatisticView(const std::shared_ptr<std::vector<cv::Mat>> mapStack, const double xView/*x*/, const double yView/*y*/, const double wView/*w*/, const double dView/*h*/, const double zRotView /*rotZ*/, const statistics stat, const std::string targetFrame, const std::string sourceFrame) {
  // Get the current data
  std::vector<cv::Mat> tmpMapStack(mapStack->size());

  tf::StampedTransform transformedViewInRoi;
  try {
    listenerTf->waitForTransform(targetFrame, sourceFrame, ros::Time(0), ros::Duration(3.0));
    listenerTf->lookupTransform(targetFrame, sourceFrame, ros::Time(0), transformedViewInRoi);
    ROS_DEBUG("getStatisticView: abs(x,y,z): %f", transformedViewInRoi.getOrigin().length());
  } catch(const std::exception &exc) {
    const std::string excStr(exc.what());
    ROS_ERROR("%s", excStr.c_str());
    throw std::runtime_error(std::string("doLaseTf: Won't perform any tf without valid machine model"));
  }
  ...
  switch (stat) {
    case statistics::mean:     getStatistic(tmpMapStack, mapStackStatistic, statistics::mean); break;
    case statistics::variance: getStatistic(tmpMapStack, mapStackStatistic, statistics::variance); break;
    case statistics::quantil90:  getStatistic(tmpMapStack, mapStackStatistic, statistics::quantil, 90); break;
    case statistics::quantil95:  getStatistic(tmpMapStack, mapStackStatistic, statistics::quantil, 95); break;
    default: throw std::runtime_error(std::string("getStatisticView: Unsupported statistic type")); break;
  }
...
///
void getStatisticOutput(const mapserver_msgs::mapPrimitive &view, mapserver_msgs::rsm &output, const std::shared_ptr<std::vector<cv::Mat>> &input, const statistics stat, const std::string &outputDimension) {
  ...
  statisticMat = getStatisticView(input, xView/*x*/, yView/*y*/, wView/*w*/, dView/*d*/, yaw /*rotZ*/, stat, targetframe, sourceframe);
...
bool mapRawServerMeanHeight(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-meanHeight method called" );
  getStatisticOutput(req.request, res.response, currentMapHeight_mm, statistics::mean, std::string("mm"));
  ROS_INFO( "rawMapserver-meanHeight method called: Finish" );
  return true;
}

bool mapRawServerVarianceHeight(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-varianceHeight method called" );
  getStatisticOutput(req.request, res.response, currentMapHeight_mm, statistics::variance, std::string("mm^2"));
  ROS_INFO( "rawMapserver-varianceHeight method called: Finish" );
  return true;
}

bool mapRawServerQuantil90Height(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-varianceHeight method called" );
  getStatisticOutput(req.request, res.response, currentMapHeight_mm, statistics::quantil90, std::string("mm"));
  ROS_INFO( "rawMapserver-varianceHeight method called: Finish" );
  return true;
}

bool mapRawServerQuantil95Height(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-varianceHeight method called" );
  getStatisticOutput(req.request, res.response, currentMapHeight_mm, statistics::quantil95, std::string("mm"));
  ROS_INFO( "rawMapserver-varianceHeight method called: Finish" );
  return true;
}

bool mapRawServerMeanPulsewidth(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-varianceHeight method called" );
  getStatisticOutput(req.request, res.response, currentPulsewidth_ps, statistics::mean, std::string("ps"));
  ROS_INFO( "rawMapserver-varianceHeight method called: Finish" );
  return true;
}

bool mapRawServerVariancePulsewidth(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-varianceHeight method called" );
  getStatisticOutput(req.request, res.response, currentPulsewidth_ps, statistics::variance, std::string("ps^2"));
  ROS_INFO( "rawMapserver-varianceHeight method called: Finish" );
  return true;
}

bool mapRawServerQuantil90Pulsewidth(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-varianceHeight method called" );
  getStatisticOutput(req.request, res.response, currentPulsewidth_ps, statistics::quantil90, std::string("ps"));
  ROS_INFO( "rawMapserver-varianceHeight method called: Finish" );
  return true;
}


bool mapRawServerQuantil95Pulsewidth(mapserver::rsm::Request &req, mapserver::rsm::Response &res) {
  ROS_INFO( "rawMapserver-varianceHeight method called" );
  getStatisticOutput(req.request, res.response, currentPulsewidth_ps, statistics::quantil95, std::string("ps"));
  ROS_INFO( "rawMapserver-varianceHeight method called: Finish" );
  return true;
}
...
void sendMap(void) {
  if (sendTopLayerOfDistanceAsOgm) {
    ...
    ogm.header.frame_id = frameTarget + std::string("_") + machine::frames::names::ROI_ORIGIN;
    ogm.info.height = mapSizeY_tiles;
    ogm.info.width = mapSizeX_tiles;
    ogm.info.resolution = resolution_meterPerTile;
    ogm.data.resize(mapSizeY_tiles * mapSizeX_tiles, 0);
    // HACK Needs parametrization for display: look at 1e-1
    for (int idx=0; idx < ogm.data.size(); ++idx) {
      ogm.data.at(idx) = int8_t(float(mapHeight_mm->at(0).at<int16_t>(idx)) * 0.1f);
...
void tfTileNameHandler(const std_msgs::String nameMsg) {
  ...
  if (currentTileTfNameChange) {
    ...
    while((!(lastMapHeight_mm.unique() && lastPulsewidth_ps.unique() && lastMapIterator.unique()) && !clearMap)  // The last map is hold by another process, but we don't care if we don't clear it
        || !(currentMapHeight_mm.unique() && currentPulsewidth_ps.unique() && currentMapIterator.unique()) && !bool(shiftMap)) { // The current map needs to be filled with the old values if shifted, but we don't care if we don't shift it
        usleep(10);
    ...
    mapRefreshAndStorage( lastPulsewidth_ps,                     // Map to shift/store/reset
                          ...
                          std::string("ps"),                     // Unit
                          resolution_meterPerTile,               // Tile resolution
...
int main(int argc, char **argv)
{
  ...
  n.param<std::string>("tile_origin_tf_sufix_for_roi_origin", tileOriginTfSufixForRoiOrigin, std::string("_") + machine::frames::names::ROI_ORIGIN);
  ...
  n.param<double>("resolution_m_tile", resolution_meterPerTile, mapping::discreteResolution);
  n.param<float>("max_distance_insertion_m", maxDistanceInsertion_m, std::min(mapping::roi::width, mapping::roi::height));
  n.param<float>("max_x_m", maxX_m, mapping::roi::xMax);
  n.param<float>("min_x_m", minX_m, mapping::roi::xMin);
  n.param<float>("max_y_m", maxY_m, mapping::roi::yMax);
  n.param<float>("min_y_m", minY_m, mapping::roi::yMin);
  n.param<int>("max_layer", maxLayer, 20);
  n.param<int>("dont_store_maps", dontStoreMaps, 0);
  n.param<int>("shift_map", shiftMap, 1);
  n.param<std::string>("map_storage_location", mapStorageLocation, "/tmp/");
  n.param<int>("send_top_layer_of_distancestack_as_ogm", sendTopLayerOfDistanceAsOgm, 1);
  n.param<double>("rate", rate, 5.0);
  ...
  subscriberLaser = n.subscribe<sensor_msgs::LaserScan>(topicLaser, 100, dataHandler);
  subscriberTfTileName = n.subscribe<std_msgs::String>(currentTfNameTopic, 2, tfTileNameHandler);

  // Prepare the map stack
  // Store discrete resolution of the map
  mapSizeX_m = (maxX_m - minX_m);
  mapSizeY_m = (maxY_m - minY_m);
  mapSizeX_tiles = mapSizeX_m / resolution_meterPerTile;
  mapSizeY_tiles = mapSizeY_m / resolution_meterPerTile;

  // Allocate space for the maps and init the double-buffer
  storageMapBuffer = cv::Mat(mapSizeX_tiles, mapSizeY_tiles, CV_8UC1);
  cv::Mat tmpTopLayerHeight = cv::Mat(mapSizeX_tiles, mapSizeY_tiles, CV_16SC1);
...

  {
    ...
    currentMapHeight_mm   = std::shared_ptr<std::vector<cv::Mat>>(new std::vector<cv::Mat>(maxLayer));
    lastMapHeight_mm      = std::shared_ptr<std::vector<cv::Mat>>(new std::vector<cv::Mat>(maxLayer));
    currentPulsewidth_ps  = std::shared_ptr<std::vector<cv::Mat>>(new std::vector<cv::Mat>(maxLayer));
    lastPulsewidth_ps     = std::shared_ptr<std::vector<cv::Mat>>(new std::vector<cv::Mat>(maxLayer));
    currentMapIterator    = std::shared_ptr<std::vector<cv::Mat>>(new std::vector<cv::Mat>(1));
    lastMapIterator       = std::shared_ptr<std::vector<cv::Mat>>(new std::vector<cv::Mat>(1));
    ...
    for (std::size_t idx = 0; idx <  maxLayer; ++idx) {
      ...
      ROS_DEBUG(" Pointer: %d", (int*)currentMapHeight_mm->at(idx).data);
    }
  ...
  const std::string s("/");
  ros::ServiceServer service_meanHeight              = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::meanHeight             , mapRawServerMeanHeight);
  ros::ServiceServer service_varianceHeight          = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::varianceHeight         , mapRawServerVarianceHeight);
  ros::ServiceServer service_quantil90Height         = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::quantil90Height        , mapRawServerQuantil90Height);
  ros::ServiceServer service_quantil95Height         = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::quantil95Height        , mapRawServerQuantil95Height);
  ros::ServiceServer service_meanPulsewidth          = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::meanPulsewidth         , mapRawServerMeanPulsewidth);
  ros::ServiceServer service_variancePulsewidth      = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::variancePulsewidth     , mapRawServerVariancePulsewidth);
  ros::ServiceServer service_quantil90Pulsewidth     = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::quantil90Pulsewidth    , mapRawServerQuantil90Pulsewidth);
  ros::ServiceServer service_quantil95Pulsewidth     = n.advertiseService(scopes::map::rawServer::parent + s + scopes::map::rawServer::requests::quantil95Pulsewidth    , mapRawServerQuantil95Pulsewidth);
