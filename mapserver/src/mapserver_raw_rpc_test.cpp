    ...
    if (client.call(req,res)){
        ...
        for(int j=0;j<res.response.cols;j++){
            for(int i=0;i<res.response.rows;i++){
                ...
                if(resVal >= 0.0f && resVal != ms::constants::numeric::invalidValue_float){
...
int main(int argc, char **argv)
{
  ...
  node.param<float>("req_resolution_meterPerTile", resolution_meterPerTile, 0.1);
  node.param<std::string>("req_frame_id", frameId, machine::frames::names::BASE_LINK);
  ...
  node.param<std::string>("req_service_name", serviceName, scopes::map::rawServer::parent + std::string("/") + scopes::map::rawServer::requests::meanHeight);
