...
void messageHandler(const nav_msgs::Odometry odomMsg,
  ...
  if (!tileCenterHistory.empty()) {  // If we don't have to bootstrap, check for shifting
    const double diffX = fabs(
        odomMsg.pose.pose.position.x
            - std::get<tupleEnum::pos>(tileCenterHistory.back()).x);
    const double diffY = fabs(
        odomMsg.pose.pose.position.y
            - std::get<tupleEnum::pos>(tileCenterHistory.back()).y);
    const double diffZ = fabs(
        odomMsg.pose.pose.position.z
            - std::get<tupleEnum::pos>(tileCenterHistory.back()).z);
    ...
    ROS_DEBUG("Odom pose x:%f, y:%f, z:%f\n"
              ...
              std::get<tupleEnum::pos>(tileCenterHistory.back()).x,
              std::get<tupleEnum::pos>(tileCenterHistory.back()).y,
              std::get<tupleEnum::pos>(tileCenterHistory.back()).z, diffX,
              diffY, diffZ);
...
pnsTuple sendTfHistory(void) {
  ...
  const std::size_t histSizeStart =
      tfTileHistory < 0 ?
          0 : currentTileCenterHistory.size() - tfTileHistory - 1;
  // Send the tf's
  for (std::size_t idx = histSizeStart; idx < currentTileCenterHistory.size();
      ++idx) {
    ...
    tileTf.child_frame_id_ = std::get<tupleEnum::name>(
        currentTileCenterHistory.at(idx));
    brTf->sendTransform(tileTf);
    brTf->sendTransform(
        tf::StampedTransform(
            utils::conversion::getTfFromEigen(roi_roiOrigin), tileTf.stamp_,
            tileTf.child_frame_id_,
            tileTf.child_frame_id_ + tileOriginTfSufixForRoiOrigin));
    ROS_DEBUG(
        "tileCenterHistory: %d/%d, x:%f, y:%f, z:%f, name:%s\n", int(idx),
        int(currentTileCenterHistory.size()),
        std::get<tupleEnum::pos>(currentTileCenterHistory.at(idx)).x,
        std::get<tupleEnum::pos>(currentTileCenterHistory.at(idx)).y,
        std::get<tupleEnum::pos>(currentTileCenterHistory.at(idx)).z,
...
int main(int argc, char **argv) {
  ...
  n.param<std::string>("tile_origin_tf_prefix", tileOriginTfPrefix,
                       "map_base_link_");
  n.param<std::string>("tile_origin_tf_sufix_for_roi_origin",
  ...
  n.param<double>("bounding_box_width", boundingBoxWidth,
                  mapping::roi::boundingbox::width);  // (m) Reset boundary in width
  n.param<double>("bounding_box_height", boundingBoxHeight,
                  mapping::roi::boundingbox::height);  // (m) Reset boundary in height
  n.param<double>("bounding_box_altitude", boundingBoxAltitude,
                  mapping::roi::boundingbox::altitude);  // (m) Reset boundary in altitude
  n.param<double>("roi_too_origin_trans_width", roiTooOriginTransWidth,
                  -mapping::roi::originWidth);  // (m) Translation in width from the center to the origin of the map in width
  n.param<double>("roi_too_origin_trans_height", roiTooOriginTransHeight,
  ...
  ROS_INFO("ROI offset x: %f, y: %f, z: %f", roiTooOriginTransWidth,
           roiTooOriginTransHeight, roiTooOriginTransAltitude);
  ROS_INFO("BB size x: %f, y: %f, z: %f", boundingBoxWidth, boundingBoxHeight,
           boundingBoxAltitude);

  if (tfNamePublishDelay < std::numeric_limits<double>::epsilon()) {
    ROS_WARN(
        "It is recommended to set tf_name_publish_delay > 0 to defy race conditions between the new tf and advertisement of using it");
  }
...
//          tileOriginTfPrefix + std::string("0")));
  ...
  ros::Publisher currentTileTfNamePublisher = n.advertise<std_msgs::String>(
      currentTfNameTopic, 1);
