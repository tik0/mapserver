 ...
 *
 */
void quaternion2euler(Eigen::Quaternion<double>* q,
...
(*euler)(0) = atan2(2.0 * (q->y() * q->z() + q->x() * q->w()),
                    (-sqx - sqy + sqz + sqw)); /** Roll **/
(*euler)(1) = asin(
    -2.0 * (q->x() * q->z() - q->y() * q->w()) / (sqx + sqy + sqz + sqw)); /** Pitch **/
(*euler)(2) = atan2(2.0 * (q->x() * q->y() + q->z() * q->w()),
                    (sqx - sqy - sqz + sqw)); /** Yaw **/
 ...
 *
 */
void euler2quaternion(Eigen::Matrix<double, NUMAXIS, 1>* euler,
...
Eigen::Quaterniond rpy2Quaternion(const double roll, const double pitch,
                                  const double yaw) {
...
void getLidarEstimations(utils::esitmatorId id, const float *scan_values,
                         std::vector<unsigned char> &isPresent, int size) {
