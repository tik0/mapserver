...
Eigen::Matrix4d roi_roiOrigin = tf::trans<double>(-mapping::roi::originWidth,
...
Eigen::Matrix4d machine_coiler = tf::trans<double>(-2.1, 1.0, 0.0)
    * tf::rotY<double>(M_PI) * tf::rotX<double>(M_PI / 2.0);

/** \brief Transformation from the Leuze sensor to the machine frame*/
Eigen::Matrix4d leuze_machine = tf::trans<double>(-2.0 * 0.852 * 1.0 / 4.0, 0.0,
                                                  0.0)
    * tf::rotY<double>(M_PI / 2.0) * tf::rotX<double>(M_PI / 2.0);
...
Eigen::Matrix4d machine_imu = tf::trans<double>(-2.593, 2.222, 0.0)
    * tf::rotX<double>(-M_PI / 2.0);

/** \brief Transformation from the machine frame to the INCLINATION frame*/
Eigen::Matrix4d machine_inclination = tf::trans<double>(-0.436691, -0.703319,
                                                        -0.0665)
    * tf::rotX<double>(-M_PI / 2.0);

/** \brief Transformation from the machine frame to the footprint of the driven axis frame*/
Eigen::Matrix4d machine_drivenAxisFootprint = tf::trans<double>(-0.436691,
                                                                -1.7456, 0.0);
...
Eigen::Matrix4d horns_lase1base = tf::trans<double>(-0.124451, -0.081424,
                                                    0.0545)
    * tf::rotY<double>(M_PI) * tf::rotX<double>(M_PI / 2.0)
    * tf::rotY<double>(-29.758 * M_PI / 180.0);
...
Eigen::Matrix4d horns_lase2base = tf::trans<double>(-0.206489, 0.062068, 0.0545)
    * tf::rotY<double>(M_PI) * tf::rotX<double>(M_PI / 2.0)
    * tf::rotY<double>(-29.758 * M_PI / 180.0);
...
Eigen::Matrix4d horns_sick1base = tf::trans<double>(-0.143544, -0.105507,
                                                    -0.230293)
    * tf::rotY<double>(-M_PI / 2.0) * tf::rotX<double>(14.613 * M_PI / 180.0);

/** \brief sick-sick_orig-c.png: Transformation from the SICK1 base plate to the SICK1 sensor frame*/
Eigen::Matrix4d sick1base_sick1 = tf::trans<double>(0.024, 0.0, 0.044)
    * tf::rotY<double>(-M_PI / 2.0) * tf::rotX<double>(M_PI / 2.0)
    * tf::rotX<double>(M_PI);

/** \brief dachhalter-sick_c.png: Transformation from the horns to the SICK2 base plate*/
Eigen::Matrix4d horns_sick2base = tf::trans<double>(-0.149531, -0.092189,
                                                    0.284711)
    * tf::rotY<double>(-M_PI / 2.0) * tf::rotX<double>(33.901 * M_PI / 180.0);
...
Eigen::Matrix4d machine_sick3 = tf::trans<double>(5.970, 1.601521, 0.166)
    * tf::rotX<double>(M_PI / 2.0)
    * tf::rotY<double>((-45 - 13.5) * M_PI / 180.0);
...
Eigen::Matrix4d ibeoBase_ibeo = tf::trans<double>(0.1, 0.0, 0.0)
    * tf::rotY<double>(30.0 * M_PI / 180.0);
...
Eigen::Matrix4d mCamBase_mCam = tf::trans<double>(0.1, 0.0, 0.0)
    * tf::rotY<double>(30.0 * M_PI / 180.0) * tf::rotY<double>(M_PI / 2.0);
...
Eigen::Matrix4d machineRoi_machine = tf::trans<double>(0.0, 0.0, 1.7456)
    * tf::rotX<double>(M_PI / 2.0) * tf::rotY<double>(M_PI);
...
Eigen::Matrix4d irCamBase_irCam = tf::trans<double>(0.1, 0.0, 0.0)
    * tf::rotY<double>(30.0 * M_PI / 180.0) * tf::rotY<double>(M_PI / 2.0);
...
Eigen::Matrix4d cCamBase_cCam = tf::trans<double>(0.1, 0.0, 0.0)
    * tf::rotY<double>(30.0 * M_PI / 180.0) * tf::rotY<double>(M_PI / 2.0);

/** \brief Transformation from the machine frame to the PF sensor*/
Eigen::Matrix4d machine_pf = tf::trans<double>(2, -0.1, 2.5)
    * tf::rotX<double>(M_PI) * tf::rotY<double>(M_PI / 2.0);

/** \brief Transformation from the machine frame to the back base plate*/
Eigen::Matrix4d machine_backBase = tf::trans<double>(5, -2.1, 0.0)
    * tf::rotX<double>(M_PI / 2.0);

/** \brief Transformation from the back base plate to the SICK3 sensor frame*/
Eigen::Matrix4d backBase_sick3 = tf::trans<double>(0.1, 0.0, 0.0)
    * tf::rotY<double>(30.0 * M_PI / 180.0);

/** \brief mcs-hok-c.png: Transformation from the machine frame to the Hokuyo base plate*/
Eigen::Matrix4d machine_hokuyoBase = tf::trans<double>(6.492219, -0.319418, 0.0)
    * tf::rotZ<double>(M_PI / 2.0) * tf::rotX<double>(-M_PI / 2.0);
...
Eigen::Matrix4d hokuyoBase_hokuyo = tf::trans<double>(0.0, 0.0, 0.1)
    * tf::rotZ<double>(M_PI / 2.0 + M_PI / 8.0 + M_PI / 16.0 - M_PI / 64.0);

/** \brief Transformation for the different Sick LDMRS layers*/
Eigen::Matrix4d sick_layer1 = tf::rotY<double>(
    -sensors::SICK_LDMRS::layer1Inclination_rad);
Eigen::Matrix4d sick_layer2 = tf::rotY<double>(
    -sensors::SICK_LDMRS::layer2Inclination_rad);
Eigen::Matrix4d sick_layer3 = tf::rotY<double>(
    -sensors::SICK_LDMRS::layer3Inclination_rad);
Eigen::Matrix4d sick_layer4 = tf::rotY<double>(
...
static std::string mapSubScopes[NUM_MAPS] { scopes::map::sub::stockDensity1,
    scopes::map::sub::stockDensity2, scopes::map::sub::stockDensity3,
    scopes::map::sub::obstacleNonProcNonAcc,
    scopes::map::sub::obstacleProcNonAcc, scopes::map::sub::obstacleNonProcAcc,
    scopes::map::sub::stockFlattened, scopes::map::sub::processed,
